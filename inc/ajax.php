...
switch ($_GET['type']) {
    ...
    case 'showseats':
        ...
        <div class="modal-content">
            ...
            echo '</div>
            <div class="col-md-6">
            Номер: '.$businfo['bus_no'].'<br/>
            Дата поїздки: '.$date.'<br/>
            Ціна: <span id="fare">' . (($fare == 0) ? '0' : ($fare-20)).  '</span> грн<br/>
            Комісія: 20 грн<br/>
            Загалом: <span id="total">'.$fare.'</span> грн
            </div>
            </div>
            </div>
            ...
            if (!isset($_GET['ticket'])) {
                ...
                <input type="submit" class="btn btn-primary" value="Купити" name="buy"/>
                </form>';
            }
            echo '<a onclick="$(\'#seatViewer\').hide()"><button type="button" class="btn btn-secondary" data-dismiss="modal">Закрити</button></a>
            </div>
            <script async>
            $(".seat[type=checkbox]").click(
                function() {
                    ...
                    $("#total").html(sts*'.$businfo['fare'].'+20);
                    let total = 0;
                    if (sts != 0)
                        total = sts*'.$businfo['fare'].'+20;
                    $("#ifare").val(total);
                }
            );
            </script>
