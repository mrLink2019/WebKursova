...
t_header("BusBook &mdash; Buy Tickets");
t_login_nav();
t_sidebar();
...
if (isset($_POST['buy'])) {
  ...
  if (!isset($_POST['seats']) || $_POST['seats'] == "") {
    ...
    if ($conn->query($sql)) {
      echo '<div class="alert alert-success">Покупка успішна!</div>';
    }
    else {
...
<form action="" method="get">
	<div class="form-group row">
      <label for="from" class="col-sm-2 col-form-label">Звідки</label>
      <div class="col-sm-7 well">
      	<input type="text" class="form-control" id="inputFrom" name="from" value="<?php echo (isset($_GET['from'])) ? $_GET['from'] : ''; ?>"/>
      </div>
  </div>
  <div class="form-group row">
    <label for="to" class="col-sm-2 col-form-label">Куди</label>
    <div class="col-sm-7">
  ...
  <div class="form-group row">
    <label for="jdate" class="col-sm-2 col-form-label">Дата поїздки</label>
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-7">
      <input type="submit" class="btn btn-info" name="submit" value="Пошук" />
    </div>
  </div>
</form>
<div class="popup" id="seatViewer"></div>
<div class="loader text-center" id="wait"><img src="img/bus_loader.gif" alt="Wait..."/></div>
<div class="table-con">
<div class="row">
  <div class="col-sm-2">ID</div>
  <div class="col-sm-4">Назва</div>
  <div class="col-sm-2">Час відбуття</div>
  <div class="col-sm-2">Час прибуття</div>
  <div class="col-sm-2">Ціна/квиток (грн)</div>
</div>
<?php
require_once 'inc/database.php';
$conn = initDB();
...
if ($res->num_rows == 0 || !isset($_GET['jdate']) || $_GET['jdate'] == '') {
  echo '<div class="row">
    <div class="col-sm-12 text-center"><h4>Немає автобусів</h4></div>
  </div>';
}
else {
